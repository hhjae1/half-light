...
| 손전등 OFF (어둡지만 안전) | 손전등 ON (밝지만 노출↑) |
|---|---|
| ![손전등 끔](docs/images/cap_stealth.gif) | ![손전등 켬](docs/images/cap_lamp.gif) |

왼쪽은 손전등을 끄고 단말기의 간접광에만 의존해 이동하는 모습(노출 낮음, 시야 제한).
오른쪽은 손전등을 켜 전방을 밝힌 모습 — 시야는 확보되지만 밝아진 만큼 좀비에게 발각되는 거리가 늘어난다.
이 **트레이드오프**가 HUD의 *감지도(노출)* 게이지로 즉시 피드백된다.
...
이동 기준 벡터와 시선 벡터를 분리해 구현했다 — View 변환을 이동 로직과 독립적으로 다루는 예.

![Alt 둘러보기](docs/images/cap_freelook.gif)
...
### 6.6 전체 플레이 데모
아래는 실제 플레이 영상(GIF)이다 — 이동/손전등/단말기/좀비 거동을 한 번에 확인할 수 있다.
