...
단말기의 빛은 어둠 속 유일한 길잡이지만, 동시에 플레이어를 비춰 **노출도(감지도)** 를 올린다.
이 트레이드오프가 HUD의 *감지도(노출)* 게이지로 즉시 피드백된다(손전등 `F` 도 같은 딜레마).
...
아래 화면에서 드럼통·도로 바리어·벽·멀리 단말기가 각각 다른 World 변환으로 한 공간에 놓여 있다.

![여러 오브젝트의 World Transform 배치](docs/images/cap_props.png)

특히 좀비는 **골격(부모-자식 본 계층)** 으로 변환이 중첩되는 대표 사례다(5장 참조).

### 2.3 자유 시점 (View Transform 응용)
`Alt` 키를 누르면 **이동 방향은 고정한 채 카메라(뷰 행렬)만 회전**한다(배틀그라운드식 둘러보기).
이동 기준 벡터와 시선 벡터를 분리해 구현했다 — View 변환을 이동 로직과 독립적으로 다루는 예다.

![달리면서 Alt로 시점만 돌려 둘러보기](docs/images/cap_freelook.gif)

### 2.4 래스터화 파이프라인
본 게임은 WebGL의 **Vertex→Rasterizer→Fragment** 래스터화 파이프라인 위에서 동작한다.
뒤에 나오는 GI도 레이트레이싱이 아니라 **이 래스터화 파이프라인으로 장면을 캡처**해 구현했다(3.2 참조).
...
게임 내 `G` 키로 즉시 토글한 **같은 지점**이다(상단 HUD에 `GI: ON` / `GI: OFF` 표기).
...
또한 `]` 키로 **GI 세기(밝기)** 를 실시간 조절할 수 있다 — 아래는 세기를 올릴수록 간접광이 강해지는 모습:

![GI 세기 실시간 조절 (])](docs/images/cap_gi_strength.gif)
...
금속 드럼통 등 매끈한 금속 재질에 **equirectangular 환경맵(PMREM)** 을 적용해 주변광 반사를 표현했다.
...
![좀비 걷기 (Walk 애니메이션)](docs/images/cap_zombie_walk.gif)
...
![발각 → 비명(Scream) → 추격 → 게임오버](docs/images/cap_chase.gif)

조명탄을 던지면 좀비가 그쪽으로 다가가 **괴로워하는(Agonize) 리액션**을 재생하며 묶인다:
...
![어둠 속 — 단말기 빛 주변을 배회하는 좀비(감지 전)](docs/images/cap_zombie_walk.gif)
...
![추격 중인 좀비의 경로 추적](docs/images/cap_chase.gif)
...
빛(단말기) 근처는 밝아 유리하지만 **방사능 피폭이 누적**된다. 너무 오래 머무르면 **치사량 피폭으로 게임오버**, 반대로 **빛에서 멀어져 어둠에 머물면 피폭이 서서히 회복**된다. → "밝음=정보·위험 / 어둠=안전"의 딜레마를 시간 축으로 강화한다.

| 단말기 근처에 오래 → 피폭 사망 | 멀리 떨어져 가만히 → 회복 |
|---|---|
| ![피폭 게임오버](docs/images/cap_rad_death.gif) | ![피폭 회복](docs/images/cap_rad_recover.gif) |

HUD의 ☢ 피폭 게이지는 **누적 피폭(영구)** 과 **현재 노출(가변)** 두 층으로 표시된다.
